...
**Run this batched across several merged features, not once per feature.** One sweep archives every plan dir that shipped since the last run, harvests all their findings and prunes all their branches in a single PR; every ~4 features, or whenever `plans/feature/` gets cluttered, is the intended rhythm.
...
If a pruned branch's feature still has a Status-ledger row in `plans/META-PLAN.md`, **leave it in place** — do not delete it. Deleting that row belongs only to a fold-back that also annotates the **Legacy → current ID map**, refreshes the **Baseline** and re-evaluates **★FOCUS** — `/run-feature` Phase C, or a full `/new-feature` run, whose Steps 3/7/8 perform the same fold-back inline (its ★FOCUS re-evaluation is scoped to the newly-added feature, not to what the just-shipped one unblocks) — and the row is `/run-feature`'s Phase C *resume trigger*: its resume table reads "PR merged + row present" as "fold the merge into META-PLAN" and "PR merged, no row" as "nothing to do", so a surviving row means neither fold-back has run for that F-ID, and deleting it here would silently skip the fold-back forever. Instead record the F-ID under a "Phase C pending" list for Step 7's report, telling the user to run `/run-feature <F-ID>`. Pruning the branch itself is still safe — `gh pr list --head feature/<slug>` matches on the PR's recorded head-ref name, not the live branch, so Phase C still resolves after the branch is gone. A hit here is a signal, not a cleanup task, and not a reason to hold back the prune. It also does not mean a sweep must follow every Phase C: run batched (see the intro), every F-ID in the batch has normally had its fold-back merged already and this paragraph never fires.
