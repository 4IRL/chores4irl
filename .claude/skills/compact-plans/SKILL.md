...
- Confirm Step 5's Status-ledger rows were left alone — three checks, each with its own pass/fail:
  ...
  - **Whole-ledger scan** → so a missed fold-back keeps surfacing on every sweep rather than only on the one that pruned its branch; passes when every hit below lands in a named report bucket. `grep -nE '^\| \*{0,2}F[0-9]+ [^|·]*\|.*\[#[0-9]+\]\(' plans/META-PLAN.md` lists every single-ID row whose PR column carries a `[#N](…)` link (`—` rows don't match; the `[^|·]*` keeps the first cell free of the `·` that joins the multi-ID superseded group, so that row is excluded by construction). For each hit take the F-ID from the first cell, `N` from `[#N](`, and the branch name from the Branch cell **with its backticks and any trailing `*(…)*` annotation stripped** (`sed -E 's/`//g; s/ *\*\(.*\)\*//; s/^ *//; s/ *$//'` on that cell) — the live ledger writes that cell as `` `feature/<slug>` ``, so a literal comparison against the raw cell would never match. Require `[[ "$N" =~ ^[0-9]+$ ]]` before using it — the same hardening Step 5 applies to `<branch>`, since this number comes from a hand-editable file rather than a `gh` field; a miss is `STOP: PR number '<N>' for <F-ID> is not numeric — refusing to build a gh call from it`. Then `gh pr view "$N" --json state,headRefName`, exit code and JSON checked as in Step 5; a call or parse failure is `STOP: gh pr view #<N> for <F-ID> failed — <raw output>`. Outcomes: `state == "MERGED"` and `headRefName` equal to the stripped branch name (or the Branch cell is `—`, in which case there is nothing to cross-check) → that F-ID is "Phase C pending"; `MERGED` with a *different* `headRefName` → the row's PR number doesn't belong to its branch — report it under "left in place" as a mis-linked row, not as Phase C pending; `CLOSED` → the row links a PR that closed unmerged — report it under "left in place" as a stale PR link (the F-ID needs `/run-feature`'s "PR closed, not merged" decision: reopen, restart, or abandon); `OPEN` → the normal in-review state, no entry. The branch cross-check is sound because `/run-feature` Phase A step 10 only records a PR it found via `gh pr list --head feature/<slug>`, so a correctly written row's link always names a PR whose head is its own Branch cell; if that step ever changes, revisit this comparison. This scan is forward-only: a row a pre-fix sweep already deleted without any fold-back is not detectable here (F14 was the only such case and its Phase C landed as PR #36).
