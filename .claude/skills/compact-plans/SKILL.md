...
If a pruned branch's feature still has a Status-ledger row in `plans/META-PLAN.md`, **leave it in place** — do not delete it. That row is `/run-feature`'s Phase C trigger: its resume table reads "PR merged + row present" as "fold the merge into META-PLAN" and "PR merged, no row" as "nothing to do", so a surviving row means Phase C has not yet run for that F-ID (the fold-back that deletes the row, annotates the Legacy → current ID map, refreshes the Baseline and re-evaluates ★FOCUS). Deleting it here would silently skip that fold-back forever. Instead record the F-ID under a "Phase C pending" list for Step 7's report, telling the user to run `/run-feature <F-ID>`. Pruning the branch itself is still safe — `gh pr list --head feature/<slug>` matches on the PR's recorded head-ref name, not the live branch, so Phase C still resolves after the branch is gone. In the intended cadence (feature PR merges → `/run-feature <F-ID>` runs Phase C → its `chore/meta-plan-update-<f-id>` PR merges → *then* this sweep) the row is already gone and this paragraph never fires; a hit is a signal, not a cleanup task.
...
- Confirm Step 5's Status-ledger rows were left alone: for each branch confirmed-pruned this run that backed an F-ID feature, run `grep -nE '^\| \*{0,2}<F-ID> ' plans/META-PLAN.md` (the row-matching form `/run-feature` Phase A step 3 uses; the trailing space after `<F-ID>` is load-bearing). No row is the expected result (Phase C already ran). A match is **not** something to delete — it means Phase C is still pending for that F-ID, so it must appear in the report's "Phase C pending" list; add it if Step 5 missed it. `git diff main -- plans/META-PLAN.md` must show no Status-ledger row removed by this sweep. A row listing several IDs is a superseded group Step 5 never prunes, so a match there means the F-ID was mis-mapped; leave it and say so.
...
Flag any mismatch before reporting. Then report: what was frozen (with SHAs/PRs), moved, deleted (tmp/, stray dirs, old ledger files — never Status-ledger rows), pruned (branches, local + remote), **Phase C pending** (each pruned F-ID whose Status-ledger row survives, with the instruction to run `/run-feature <F-ID>`), and left in place (any Step 5 candidate not pruned — no merged PR of its own, or a verification `SKIP:` — with the reason, for the user to handle manually). Then `/git-commit` on the working branch, followed by `/git-push` — runs the 8-agent review and opens/updates the PR before anything from this sweep is pushed, matching `/run-feature`'s own review-and-push gate (which `/worktree` also relies on via its hand-off to `/run-feature`). If it rejects, fix per its findings and re-push; a plans-only sweep is still exactly the kind of change that gate exists for.

## Important Notes

- Reorganizes planning docs only — never source files, never plans/implements a feature.
- Never deletes a `plans/META-PLAN.md` Status-ledger row — that is `/run-feature` Phase C's job and its trigger. Intended cadence per feature: feature PR merges → `/run-feature <F-ID>` (Phase C) → its META-PLAN PR merges → this sweep. A surviving row at Step 5 is reported as "Phase C pending", never cleaned up here.
