  ...
  if [[ -z "$merged_sha" ]]; then
    ...
    if [[ "$local_done" == yes ]]; then   # only reached if the local branch was deleted just now or was already absent
      if remote_tip=$(gh api "repos/4IRL/chores4irl/git/ref/heads/<branch>" --jq .object.sha 2>&1); then   # singular /ref/ = exact match; plural /refs/ prefix-matches
        ...
        echo "remote ref for <branch> already gone — skipping DELETE (benign no-op)"   # for a remote-only entry this means the branch is fully gone everywhere; report it as already-clean, not as pruned
...
```
**Stale-ref cleanup after each branch that reached the remote leg** (a successful DELETE *or* the benign `HTTP 404`): `git fetch --prune` never runs in this shell (SSH), so the local remote-tracking ref `refs/remotes/origin/<branch>` lingers and keeps re-appearing in `git branch -a` as a phantom candidate on every future sweep — delete it with `git branch -dr "origin/<branch>"` (local bookkeeping only; a "remote-tracking branch not found" error is fine). And under the sandbox, `git branch -D` prints `error: could not lock config file .git/config` and leaves its `[branch "<branch>"]` section behind (the branch itself is deleted) — run `git config --remove-section "branch.<branch>"` for each pruned branch in one Bash call with `dangerouslyDisableSandbox: true` (it only touches `.git/config`; "no such section" is fine for a remote-only entry, which never had one). Note in the report which remote refs were already gone (404) versus deleted here — GitHub's auto-delete-on-merge may already have removed them.

A **remote-only** candidate — a `remotes/origin/<branch>` in `git branch -a` with a merged PR but no local `refs/heads/<branch>` — is a normal case, not a SKIP: `/worktree teardown` deletes a merged branch locally but deliberately leaves its remote ref for this step, so every worktree-driven feature arrives here remote-only. Its candidate list must therefore be built from the *remote* refs too (`git branch -r`, stripping the `origin/` prefix), not just local ones — a local-only cross-check would never see it. The remote leg's `remote_tip == head_oid` check is the only tip gate such an entry gets, which is sufficient: it proves the remote ref holds exactly what the merged PR merged and nothing more.
