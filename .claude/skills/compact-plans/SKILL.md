...
- Confirm Step 5's Status-ledger rows were left alone — three checks, each with its own pass/fail:
  - `git diff HEAD --quiet -- plans/META-PLAN.md` should succeed (exit 0). This sweep only ever *reads* that file and the Branch Guard required a clean tree at the start, so any uncommitted change to it here is the sweep's own doing — stop and report the diff rather than reverting it silently. The check does not depend on how fresh local `main` is or which branch the sweep runs on.
  ...
  - Whole-ledger scan, so a missed fold-back keeps surfacing on every sweep rather than only on the one that pruned its branch: `grep -nE '^\| \*{0,2}F[0-9]+ [^|·]*\|.*\[#[0-9]+\]\(' plans/META-PLAN.md` lists every single-ID row whose PR column carries a `[#N](…)` link (`—` rows don't match; the `[^|·]*` keeps the first cell free of the `·` that joins the multi-ID superseded group, so that row is skipped by construction — verified against the live ledger plus synthetic linked rows). For each hit take the F-ID from the first cell and `N` from `[#N](`, and require `[[ "$N" =~ ^[0-9]+$ ]]` before using it — the same hardening Step 5 applies to `<branch>`, since this number comes from a hand-editable file rather than a `gh` field; a miss is `STOP: PR number '<N>' for <F-ID> is not numeric — refusing to build a gh call from it`. Then `gh pr view "$N" --json state,mergedAt,headRefName`, exit code and JSON checked as in Step 5; a call failure is `STOP: gh pr view <N> for <F-ID> failed: <raw error>`. `state == "MERGED"` with `headRefName` equal to the row's Branch cell means that F-ID is "Phase C pending" too; `MERGED` with a *different* `headRefName` means the row's PR number doesn't belong to its branch — report it under "left in place" as a mis-linked row rather than as Phase C pending; `OPEN` is the normal in-review state and needs no entry. This scan is forward-only: a row a pre-fix sweep already deleted without any fold-back is not detectable here (F14 was the only such case and its Phase C landed as PR #36).
