...
# for every branch whose local `git branch -D` succeeded this run — regardless of how its remote leg ended (a SKIP/STOP there still leaves this section behind)
if ! out=$(git config --remove-section "branch.<branch>" 2>&1); then
  if [[ "$out" == *"no such section"* ]]; then echo "no config section for branch.<branch> (benign)"   # a remote-only entry, or `-D` ran unsandboxed and cleaned it itself
...
- Confirm Step 5's stale-ref cleanup landed: `git config --get-regexp '^branch\.'` must list no `branch.<name>.*` key for any branch pruned this run, and `git branch -r` no `origin/<name>` for any whose remote leg reached DELETE or 404 — a hit means that cleanup call was skipped or ran sandboxed; run it now (with the flag) and report it.
