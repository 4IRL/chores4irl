...
**This sweep is batched across several merged features — it is not a per-feature step.** One run archives every plan dir whose feature has shipped since the last sweep, harvests all their findings, and prunes all their branches in a single PR. Sweeping every ~4 features (or whenever `plans/feature/` gets cluttered) is the intended rhythm; a sweep per feature would add a third PR to every feature's cadence for no gain.

Two reasons the batched form is the right default, not merely the cheaper one:

- **The safety net is frequency-independent.** Step 7's whole-ledger scan is *forward-only* — a missed fold-back resurfaces on **every** later sweep, not only the one that pruned its branch. Sweeping less often delays that catch; it does not weaken it.
- **`/worktree` requires it.** Parallel features hand off to one `/run-feature` per worktree, and teardown deliberately leaves remote refs for Step 5 to clean up. After a parallel batch you want *one* sweep covering N features, not N sweeps racing each other's branch prunes and `git fetch`es.

The one hard ordering rule: never sweep an F-ID's branch before that F-ID's fold-back has merged (see Step 5's Status-ledger paragraph). That gates *which* F-IDs a given sweep may prune — it does not gate how often the sweep runs.
...
```
(`Abandoned — <reason>` / `Superseded by <X>` as appropriate.) The one line that earns its keep is *why* — that's what git alone can't tell a future session. A sidecar `.md` at the plan's top level (e.g. the `pr-description.md` that `/run-feature` saves beside the plan) gets the same first line followed by a pointer to the main plan instead of an Outcome — Step 7's header check covers every top-level `.md`, not just the plan.
...
if ! pr_json=$(gh pr view <number> --json state,mergedAt,mergeCommit,headRefName,headRefOid 2>&1); then   # the call itself failed (auth/rate-limit/network) — a STOP, not a SKIP: an empty read below could not tell that apart from "not merged"
  ...
  else   # the PR gate passed — now the local leg, then the remote leg
    local_done=no
    if git rev-parse --verify --quiet "refs/heads/<branch>" >/dev/null; then   # checked before merge-base, which exits 128 (not 1) on a missing ref and would misreport the tip check
      if ! git merge-base --is-ancestor "<branch>" "$head_oid"; then
        echo "SKIP: local <branch> tip is beyond PR #<number>'s head $head_oid — no local or remote action; continue to the next branch"
      elif git branch -D "<branch>"; then   # -D on purpose — see below
        local_done=yes
      else
        echo "SKIP: local delete failed for <branch> — no remote action; investigate, then continue to the next branch"
      fi
    else   # remote-only: no local <branch> — already deleted (e.g. by /worktree teardown, which never touches remote refs) — so there is no local tip to compare; the remote leg's own tip == head_oid check is the tip gate here
      echo "remote-only: no local <branch> (already deleted) — proceeding to its remote ref for PR #<number>"
      local_done=yes
    fi
    if [[ "$local_done" == yes ]]; then   # only reached if the local branch was deleted just now or was already absent
      if remote_tip=$(gh api "repos/4IRL/chores4irl/git/ref/heads/<branch>" --jq .object.sha 2>&1); then   # singular /ref/ = exact match; plural /refs/ prefix-matches
        ...
        elif ! gh api -X DELETE "repos/4IRL/chores4irl/git/refs/heads/<branch>"; then   # only reached once the remote ref was confirmed present at the PR's head
          echo "STOP: remote delete failed for <branch> — report and do not continue to the next branch"
        fi
      elif [[ "$remote_tip" == *"HTTP 404"* ]]; then
        echo "remote ref for <branch> already gone — skipping DELETE (benign no-op)"   # still a "pruned" entry in Step 7's report — annotate its remote leg "already gone (404)" rather than "deleted here"
...
```
**Stale-ref cleanup**, run once after the per-branch loop over the same `<branch>` names the loop already passed through its regex gate (never a name from anywhere else), in a single Bash call with `dangerouslyDisableSandbox: true` (the `git config` line writes `.git/config`, which the sandbox denies — that denial is also why the loop's `git branch -D` prints `error: could not lock config file .git/config` and leaves its `[branch "<branch>"]` section behind even though the branch itself is deleted). Two leftovers per pruned branch, each with exactly one benign error and a `STOP:` for anything else — the same shape as the loop's `HTTP 404` branch:
```bash
# for every branch whose remote leg ended in a DELETE or the benign HTTP 404: `git fetch --prune` never runs in this shell (SSH), so refs/remotes/origin/<branch> lingers and re-appears in `git branch -a` as a phantom candidate on every future sweep
if ! out=$(git branch -dr "origin/<branch>" 2>&1); then
  if [[ "$out" == *"remote-tracking branch 'origin/<branch>' not found"* ]]; then echo "no local tracking ref for origin/<branch> (benign)"   # a local-only entry, or already pruned
  else echo "STOP: could not delete tracking ref origin/<branch> — $out"; fi
fi
# for every branch whose local `git branch -D` succeeded this run — regardless of how its remote leg ended (a SKIP/STOP there still leaves this section behind) — and for every remote-only entry (a sweep halted by a STOP after its `-D` can leave the section for a later run to find as remote-only)
if ! out=$(git config --remove-section "branch.<branch>" 2>&1); then
  if [[ "$out" == *"no such section"* ]]; then echo "no config section for branch.<branch> (benign)"   # `-D` ran unsandboxed and cleaned it itself, or a remote-only entry that never had one
  else echo "STOP: could not remove config section branch.<branch> — $out"; fi   # e.g. `could not lock config file` — the call ran sandboxed; re-run it with the flag
fi
```

A **remote-only** candidate (no local `refs/heads/<branch>`) is a normal case, not a SKIP — every worktree-driven feature arrives here that way. The remote leg's `remote_tip == head_oid` check is the only tip gate such an entry gets, and it is sufficient: it proves the remote ref holds exactly what the merged PR merged and nothing more.
...
If a pruned branch's feature still has a Status-ledger row in `plans/META-PLAN.md`, **leave it in place** — do not delete it. Deleting that row belongs only to a fold-back that also annotates the **Legacy → current ID map**, refreshes the **Baseline** and re-evaluates **★FOCUS** — `/run-feature` Phase C, or a full `/new-feature` run, whose Steps 3/7/8 perform the same fold-back inline (its ★FOCUS re-evaluation is scoped to the newly-added feature, not to what the just-shipped one unblocks) — and the row is `/run-feature`'s Phase C *resume trigger*: its resume table reads "PR merged + row present" as "fold the merge into META-PLAN" and "PR merged, no row" as "nothing to do", so a surviving row means neither fold-back has run for that F-ID, and deleting it here would silently skip the fold-back forever. Instead record the F-ID under a "Phase C pending" list for Step 7's report, telling the user to run `/run-feature <F-ID>`. Pruning the branch itself is still safe — `gh pr list --head feature/<slug>` matches on the PR's recorded head-ref name, not the live branch, so Phase C still resolves after the branch is gone. The constraint this paragraph encodes is an **ordering** one, not a frequency one: a given F-ID's row must already be gone before this sweep touches its branch, i.e. that F-ID's Phase C (or `/new-feature` fold-back) has merged. It does **not** mean a sweep follows every Phase C — see *Sweep frequency* above. In a normal batched run every F-ID in the batch has already had its fold-back merged, so this paragraph never fires; a hit is a signal, not a cleanup task.
...
- Re-run Step 5's `gh pr list --state merged --limit 500 --json number,title,headRefName,mergeCommit` + `git branch -a` cross-check and confirm every branch confirmed for pruning is actually gone (local and remote — for a remote-only entry, that its `remotes/origin/<branch>` ref is gone after a `git fetch --prune`-equivalent refresh, e.g. `git remote prune origin` or a `gh api …/git/ref/heads/<branch>` returning `HTTP 404`), and no **Live** / open-PR branch was touched.
- Confirm Step 5's stale-ref cleanup landed: `git config --get-regexp '^branch\.'` must list no `branch.<name>.*` key for any branch pruned this run (either leg), and `git branch -r` no `origin/<name>` for any whose remote leg reached DELETE or 404 — a hit means that cleanup call was skipped, ran sandboxed, or was out of scope; run it now (with the flag) and report it.
- Confirm Step 5's Status-ledger rows were left alone — three checks, each with its own pass/fail:
  - **Uncommitted diff** → `git diff HEAD --quiet -- plans/META-PLAN.md` should succeed (exit 0). This sweep only ever *reads* that file and the Branch Guard required a clean tree at the start, so any uncommitted change to it here is the sweep's own doing — stop and report the diff rather than reverting it silently. The check does not depend on how fresh local `main` is or which branch the sweep runs on.
  - **Pruned-branch rows** → for each branch confirmed-pruned this run that backed an F-ID feature, `grep -nE '^\| \*{0,2}<F-ID> ' plans/META-PLAN.md` should return nothing (the row-matching form `/run-feature` Phase A step 3 uses; the trailing space after `<F-ID>` is load-bearing) — no row means a fold-back already ran. A match is **not** something to delete: it means Phase C is still pending for that F-ID, so it must appear in the report's "Phase C pending" list — add it if Step 5 missed it. A row listing several IDs is a superseded group Step 5 never prunes, so a match there means the F-ID was mis-mapped; leave it and report it under "left in place".
  - **Whole-ledger scan** → so a missed fold-back keeps surfacing on every sweep rather than only on the one that pruned its branch; passes when every hit below lands in a named report bucket. `grep -nE '^\| \*{0,2}F[0-9]+ .*\[#[0-9]+\]\(' plans/META-PLAN.md | grep -vE '^[0-9]+:\| \*{0,2}F[0-9]+ · '` lists every single-ID row whose PR column carries a `[#N](…)` link (`—` rows don't match; the second filter drops the multi-ID superseded group, whose first cell joins its IDs as `F3 · F7 · …` — a `·` elsewhere in a title does not match `F<n> · `). The scan is safe to run over the whole file only because `[#N](` link syntax appears solely in the Status ledger's PR column today; a future table reusing that shape is a reason to revisit the grep. For each hit, `lineno` is the number the `-n` prefix printed; then `row=$(sed -n "${lineno}p" plans/META-PLAN.md)`, `fid=$(grep -oE 'F[0-9]+' <<<"$row" | head -1)`, `N=$(grep -oE '\[#[0-9]+\]\(' <<<"$row" | grep -oE '[0-9]+')`, and `branch=$(awk -F'|' '{print $4}' <<<"$row" | sed -E 's/`//g; s/ *\*\(.*\)\*//; s/^ *//; s/ *$//')` — the Branch cell **with its backticks and any trailing `*(…)*` annotation stripped** (`` `feature/x` `` → `feature/x`; `` `feature/x` *(pruned locally …)* `` → `feature/x`; `—` → `—`), because the live ledger writes that cell as `` `feature/<slug>` `` and a literal comparison against the raw cell would never match. Require `[[ "$N" =~ ^[0-9]+$ ]]` before using it — the same hardening Step 5 applies to `<branch>`, since this number comes from a hand-editable file rather than a `gh` field; a miss is `STOP: PR number <N> for <F-ID> is not exactly one numeric value — refusing to build a gh call from it` (a row carrying two `[#N](` links lands here too: the extraction then yields two lines, which the anchored regex rejects; the row-writing contract puts exactly one link per row, so a hit means a hand-edit). Then `gh pr view "$N" --json state,headRefName` (`<F-ID>`/`<N>` below are `$fid`/`$N`), exit code and JSON checked as in Step 5; a call or parse failure is `STOP: gh pr view #<N> for <F-ID> failed — <raw output>`. Outcomes: `state == "MERGED"` and `headRefName` equal to `$branch` (or the Branch cell is `—`, in which case there is nothing to cross-check — defensive only; the row-writing contract always populates Branch at creation) → that F-ID is "Phase C pending"; `MERGED` with a *different* `headRefName` → the row's PR number doesn't belong to its branch — report it under "left in place" as a mis-linked row, not as Phase C pending; `CLOSED` → the row links a PR that closed unmerged — report it under "left in place" as a stale PR link (the F-ID needs `/run-feature`'s "PR closed, not merged" decision: reopen, restart, or abandon); `OPEN` → the normal in-review state, deliberately no entry. The branch cross-check is sound because `/run-feature` Phase A step 10 only records a PR it found via `gh pr list --head feature/<slug>`, so a correctly written row's link always names a PR whose head is its own Branch cell; if that step ever changes, revisit this comparison. This scan is forward-only: a row a pre-fix sweep already deleted without any fold-back is not detectable here (F14 was the only such case and its Phase C landed as PR #36).
...
Flag any mismatch before reporting. Then report: what was frozen (with SHAs/PRs), moved, deleted (tmp/, stray dirs, old ledger files — never Status-ledger rows), pruned (branches, each annotated per leg — local `deleted (was <sha>)` / `already absent`, remote `deleted here` / `already gone (404)` — so a remote-only or auto-deleted-on-merge entry still lands here rather than in an undefined bucket), "Phase C pending" (each F-ID whose Status-ledger row survives a merged PR — from the pruned-branch grep or the whole-ledger scan — with the instruction to run `/run-feature <F-ID>`), and "left in place" (any Step 5 candidate not pruned — no merged PR of its own, or a verification `SKIP:` — with the reason, for the user to handle manually; plus any Step 7 grep hit that was a superseded-group row rather than a Phase C trigger, and any scan hit whose PR doesn't belong to its branch or closed unmerged — each naming the F-ID and what was matched; OPEN-linked rows are deliberately unreported). Then `/git-commit` on the working branch, followed by `/git-push` — runs the 8-agent review and opens/updates the PR before anything from this sweep is pushed, matching `/run-feature`'s own review-and-push gate (which `/worktree` also relies on via its hand-off to `/run-feature`). If it rejects, fix per its findings and re-push; a plans-only sweep is still exactly the kind of change that gate exists for.
