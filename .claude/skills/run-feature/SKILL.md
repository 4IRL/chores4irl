...
3. Set this feature's Status-ledger row in `plans/META-PLAN.md` to `in-progress` (a small standalone edit that rides in the first implementation commit). **Verify it landed** — `grep -nE '^\| \*{0,2}<F-ID> ' plans/META-PLAN.md` returns exactly one row, this feature's, whose second (Status) column reads `in-progress` (`\*{0,2}` covers the bolded ★FOCUS row; the trailing space after `<F-ID>` is load-bearing — it keeps `F1` from matching `F14`/`F15`/`F1-L`), and `git diff -- plans/META-PLAN.md` shows that row as the only changed line. If not, fix the edit before step 4 — unless the row was already absent before your edit, which is a ledger gap to stop and report, not a row to invent.
...
8. **Verify "Expected end state"** — run the relevant Vitest suites + `e2e/smoke.spec.ts`, and check every listed grep/route fact. Reconcile any gap before publishing. **When `git worktree list` shows any `c4i-wt-<slug>` worktree** (this checkout is one of them, or it's the main checkout with siblings), run the smoke spec as `env -u PLAYWRIGHT_BASE_URL CI=1 npx playwright test e2e/smoke.spec.ts`: `playwright.config.ts` hardcodes its two `webServer` entries on `localhost:3000` and `localhost:5174` with `reuseExistingServer: !process.env.CI`, so without `CI` set, a smoke run that starts while another checkout's dev servers are already listening silently adopts them and verifies the wrong build (and an inherited `PLAYWRIGHT_BASE_URL` disables `webServer` entirely, making `CI=1` inert — hence the `env -u`). With `CI=1` that run instead fails loudly — `… is already used, make sure that nothing is running on the port/url …`, or `Process from config.webServer was not able to start` when two runs launched within the same few seconds — which means something else holds the ports, usually a sibling's smoke run in progress: retry about every 30 s (jitter the delay so two waiters don't relaunch together) for up to ~10 min; if it still fails, stop and report the ports as occupied (`ss -ltnp` names the listener read-only, where available). Never kill the listener, free the port, drop `CI=1`, skip verification, or edit `playwright.config.ts` to get past it — parameterizing the ports (and `strictPort` for Vite) is app code, an optional follow-up F-ID outside this step.
...
10. Update the Status-ledger row to `in-review` with the now-known PR link, in a small follow-up commit — matching `META-PLAN.md`'s own Ledger update protocol ("in-review + PR link after git-push"). **Verify it landed** — re-run step 3's grep (and its `git diff` check; the tree is otherwise clean after step 9) and confirm the same row's Status column now reads `in-review` and its fourth (PR) column carries this PR's link in the table's `[#N](https://github.com/4IRL/chores4irl/pull/N)` form, `N` being the PR step 9 opened or updated — not the `—` placeholder; confirm `N` with `gh pr list --head feature/<slug> --state open --json number,url` (exit code and JSON checked as under "Decide which phase to resume"), and if that call can't establish `N`, stop and report rather than commit. Reconcile any gap before committing. Commit, then `git push` directly to land it on the already-open PR (a plain push, not another full `/git-push` review cycle — that PR was already reviewed in step 9 and this is a trivial one-line follow-up); confirm the push actually succeeded — check `git push`'s own exit status/output, or re-run `git status` and confirm it reports nothing to push; on failure, stop and report rather than falling through to Phase B.
