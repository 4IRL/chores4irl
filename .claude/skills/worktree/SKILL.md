...
### 1. Preconditions
Require `main` clean and synced (`git status`, `git branch --show-current`; `gmas` if behind). A dirty or non-`main` base silently poisons every worktree — stop and fix this first, don't work around it.

### 2. Resolve and filter the F-id set
...
### 3. Compute touch-sets live — never reuse a cached table
...
### 4. Pairwise independence matrix
...
If either branch in a pair already has commits, sharpen with the empirical check:
```bash
git merge-tree --write-tree feature/<a> feature/<b> | grep -q '^CONFLICT' && echo "CONFLICT <a>/<b>" || echo "clean"
```
Any reported conflict downgrades that pair to RED regardless of the static call.

Print the full matrix and each feature's touch-set — the independence claim must be auditable, not asserted.

**Pause-and-ask checkpoint:** any **RED** pair → stop, report it, recommend running those two serially (drop one from this batch, or abort). Any **YELLOW** pair → `AskUserQuestion`: accept the risk (proceed, flag it as needing a rebase check at merge time) or drop one of the pair. Only GREEN and accepted-YELLOW features proceed to Step 5.
...
```
Then, in each worktree: `npm install` (node_modules is per-worktree, not shared), and confirm it's on the right branch and clean.
...
- After each merge, remaining worktree branches should `git fetch && git rebase origin/main` and re-run their suites; re-run the Step 4 `merge-tree` check between any two not-yet-merged branches if either rebased.
...
2. For each, check its branch's PR state (`gh pr list --head <branch> --state all`). Candidates to remove: PR merged, or the user names it abandoned.
3. **Pause-and-ask checkpoint:** present the exact list (worktree path, branch, PR state) via `AskUserQuestion` before removing anything.
4. For each confirmed entry:
   ```bash
   git worktree remove ../c4i-wt-<slug>   # refuses if dirty — resolve first, don't force
   git branch -d feature/<slug>            # -D only if the user explicitly confirmed abandoned
   ```
