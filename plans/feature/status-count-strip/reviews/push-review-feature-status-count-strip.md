...
- [ ] **Collapse the redundant `grow` field** — `frontend/src/components/nav/StatusCountStrip.tsx` — Drop `grow` from the `shown` entries and render `style={{ flexGrow: segment.count || 1, flexBasis: 0 }}`. This is safe because `count` is 0 only in the single-segment all-zero fallback. Keep the component test's `flexGrow === '1'` all-zero assertion green.
