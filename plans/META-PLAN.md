...
- **Chore-list track:** only `F5` remains. `F14` (#34) and `F4` (#38) shipped in the
  ...
  Name (`clearable`), the Room `<datalist>`, Last Completed, Duration and Frequency, and
  `F5` touches only the `App.tsx` footer deck — no shared-form surface at all.
...
`feature/translucent-add-deck` as of 2026-09-19 — `/run-feature F5` resumes from that PR's
state, not from Phase A). **After `F5` merges the chore-list track is empty:** the kiosk
...
- **Do not** re-open `F4`'s or `F14`'s scope — Details / Long-term are gone (#38, with the
  `db.ts` boot migration) and the clear-✕ affordance shipped (#34); Standing invariants
...
|---|---|---|---|---|
| ★ | **F5** — Translucent/blur *Add Task* deck **[FOCUS]** | **S** | — *(F4 and F14 already merged)* | Chore-list |
...
3. **Resolved (`F4`, #38):** `details` was never rendered, and its removal shipped without a display change; the one user-visible change was the sort (long-term chores no longer pin to the bottom — Standing invariant 11).
...
- **Cross-feature couplings to honor:**
  ...
  - **F14 ↔ F4 / F4 ↔ F3-L — retired (all shipped).** `F4` (#38) deleted the Details
    `FormField` and the Long-term checkbox with no diff to `ClearButton.tsx`,
    `FormField.tsx`, `ChoreSearchInput.tsx` or the Room `<datalist>`; Standing invariants
    6 and 10 were verified intact after the merge.
