...
`frontend/src/components/common/TouchLockOverlay.tsx` — a full-viewport `z-[90]` portal that
`App.tsx` mounts only on a guarded attempt (`lockAttempt`), seeded with it as the first tap; a
second tap within `SECOND_TAP_WINDOW_MS = 1500` and `SECOND_TAP_MAX_DISTANCE_PX = 60`
unlocks (`CLOSING_SETTLE_MS = 400` handshake, carried by `lockAttempt`); otherwise it fades
